...
Func cmbProfile()
	If $ichkIdleAfter = 1 Then
		If $iGoldCurrent >= $itxtIdleGoldLimit And $iElixirCurrent >= $itxtIdleElixerLimit And $iDarkCurrent >= $itxtIdleDELimit Then
			Setlog("Gold and Elixer is at the limit, Profile set to idle")
			GUICtrlSetState($radActiveProfile, $GUI_UNCHECKED)
			GUICtrlSetState($radDonateProfile, $GUI_UNCHECKED)
	...
	;Reset Some Values After a Profile Switch
		$FirstStart = True
		;Reset Barrack values
			For $i = 0 to 3
				$BarrackStatus[$i] = False
				$BarrackFull[$i] = False
			Next
			For $i = 0 to 1
				$BarrackDarkStatus[$i] = False
				$BarrackDarkFull[$i] = False
			Next
		;Set troop donated values to zero
			;Elixer Troops
...
EndFunc   ;==>chkUseTrainingClose
 ...
 Func chkRestartAndroidSearchLimit()
	...
	EndIf
 EndFunc   ;==>chkRestart Android

  Func chkIdleAfter()
	If GUICtrlRead($chkIdleAfter) = $GUI_CHECKED Then
	   GUICtrlSetState($lblIdleafter1, $GUI_ENABLE)
	   GUICtrlSetState($txtIdleGoldLimit, $GUI_ENABLE)
	   GUICtrlSetState($lblIdleafter2, $GUI_ENABLE)
	   GUICtrlSetState($lblIdleafter3, $GUI_ENABLE)
	   GUICtrlSetState($txtIdleElixerLimit, $GUI_ENABLE)
	   GUICtrlSetState($lblIdleafter4, $GUI_ENABLE)
	   GUICtrlSetState($lblIdleafter5, $GUI_ENABLE)
	   GUICtrlSetState($txtIdleDELimit, $GUI_ENABLE)
	Else
	   GUICtrlSetState($lblIdleafter1, $GUI_DISABLE)
	   GUICtrlSetState($txtIdleGoldLimit, $GUI_DISABLE)
	   GUICtrlSetState($lblIdleafter2, $GUI_DISABLE)
	   GUICtrlSetState($lblIdleafter3, $GUI_DISABLE)
	   GUICtrlSetState($txtIdleElixerLimit, $GUI_DISABLE)
	   GUICtrlSetState($lblIdleafter4, $GUI_DISABLE)
	   GUICtrlSetState($lblIdleafter5, $GUI_DISABLE)
	   GUICtrlSetState($txtIdleDELimit, $GUI_DISABLE)
	EndIf
 EndFunc   ;==>chk Idle After
