...
Func SwitchProfile($SwitchCase) 										; Switch profile (1 = Active, 2 = Donate, 3 = Stay, 4 = switching continuosly) - DEMEN
   ...
   Case 4
	...
	Case 5 ; Used with Delay Clan War Prep
	 Setlog("Switching to next account")
	 $NextProfile = 1
	 If $nCurProfile < $nTotalProfile Then
		$NextProfile = $nCurProfile + 1
...
Func SwitchCOCAcc()
  ...
  ; If _Sleepstatus(10000) Then Return
	Local $InCurrentAccount = 0
	Local $NotCurrentAccount = 0
	$idx = 0
	While $idx <= 30

		If _ColorCheck(_GetPixelColor(380, 430, True), Hex(0x60AE20, 6), 20) Then
			...
			ExitLoop
		EndIf
		If _ColorCheck(_GetPixelColor(580, 450, True), Hex(0x68B010, 6), 20) Then
			Setlog("Load button appears")
			$NotCurrentAccount = 1
			ExitLoop
		EndIf
		If ($idx/3) = Int($idx/3) Then Setlog("Wait!")
		If _Sleep(1000) Then Return
		$idx = $idx + 1
	WEnd
	If $NotCurrentAccount = 0 And $InCurrentAccount = 0 Then
		Setlog("Switch Account Timed Out, Restarting CoC To Try Again", $COLOR_RED)
		$bReMatchAcc = True
		WaitnOpenCoC(5000,True)
		runBot()
	EndIf
	If $NotCurrentAccount = 1 Then
		$idx = 0
